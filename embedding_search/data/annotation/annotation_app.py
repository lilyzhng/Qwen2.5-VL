...
def load_annotation_data():
    ...
    parquet_file = project_root / "data" / "unified_embeddings.parquet"
...
def main():
    ...
    if len(videos_to_annotate) == 0:
        ...
        if 'review_annotations' not in st.session_state:
            st.session_state.review_annotations = {}
        if 'review_annotations_loaded' not in st.session_state:
            st.session_state.review_annotations_loaded = False
        
        # Mode selector
        st.subheader("📋 Review Mode")
        review_mode = st.radio(
            "Choose review mode:",
            options=['summary', 'annotations_review'],
            format_func=lambda x: {
                'summary': '📊 Dataset Summary',
                'annotations_review': '✏️ Annotations Review'
            }[x],
        ...
        if st.session_state.review_mode == 'summary':
        ...
        elif st.session_state.review_mode == 'annotations_review':
            # Annotations review mode with editing capability
            # st.subheader("✏️ Annotations Review & Edit")
            
            # Get fully annotated videos
            semantic_columns = ['pv_object_type', 'pv_actor_behavior', 'pv_spatial_relation', 'ego_behavior', 'scene_type']
            fully_annotated_mask = True
            for col in semantic_columns:
                if col in full_df.columns:
            ...
            annotated_df = full_df[fully_annotated_mask].copy().reset_index(drop=True)
            
            if len(annotated_df) == 0:
                st.warning("No fully annotated videos found.")
                return
            ...
            with col_search:
                search_term = st.text_input(
                    "Search annotations (keywords, slice_id, etc.):",
                    value=st.session_state.search_filter,
                    placeholder="e.g., pedestrian, intersection, ego turning..."
                )
            ...
            if st.session_state.search_filter:
                search_lower = st.session_state.search_filter.lower()
                filtered_mask = (
                    annotated_df['slice_id'].astype(str).str.lower().str.contains(search_lower, na=False) |
                    annotated_df['pv_object_type'].astype(str).str.lower().str.contains(search_lower, na=False) |
                    annotated_df['pv_actor_behavior'].astype(str).str.lower().str.contains(search_lower, na=False) |
                    annotated_df['pv_spatial_relation'].astype(str).str.lower().str.contains(search_lower, na=False) |
                    annotated_df['ego_behavior'].astype(str).str.lower().str.contains(search_lower, na=False) |
                    annotated_df['scene_type'].astype(str).str.lower().str.contains(search_lower, na=False)
                )
                filtered_df = annotated_df[filtered_mask].reset_index(drop=True)
                
                if len(filtered_df) == 0:
                    st.warning(f"No annotations found matching '{st.session_state.search_filter}'")
                    return
                else:
                    st.info(f"Found {len(filtered_df)} videos matching '{st.session_state.search_filter}'")
                    annotated_df = filtered_df
            
            # Ensure review_index is within bounds
            if st.session_state.review_index >= len(annotated_df):
                st.session_state.review_index = len(annotated_df) - 1 if len(annotated_df) > 0 else 0
            if st.session_state.review_index < 0:
                st.session_state.review_index = 0
            
            # Progress for review
            if len(annotated_df) > 0:
                progress_value = (st.session_state.review_index + 1) / len(annotated_df)
                progress_value = max(0.0, min(1.0, progress_value))
                st.progress(progress_value)
                st.write(f"Reviewing: {st.session_state.review_index + 1} / {len(annotated_df)} videos")
            
            # Current video for review
            current_review_video = annotated_df.iloc[st.session_state.review_index]
            
            # Layout: GIF on left, annotations on right
            col1, col2 = st.columns([2, 1])
            
            with col1:
                st.subheader(f"📹 {current_review_video['slice_id']}")
                
                # Display GIF
                gif_path = current_review_video['gif_path']
                if pd.notna(gif_path) and gif_path:
                    display_gif(gif_path)
                else:
                    st.warning("No GIF available for this video")
                    st.write(f"Video path: {current_review_video['video_path']}")
            ...
            with col2:
                ...
                with col_save:
                    ...
                    if st.button("💾 Save Changes", type="primary", help="Save annotation changes", key="review_save"):
                        # Update the full_df with changes from review
                        for review_idx, annotations in st.session_state.review_annotations.items():
                            if isinstance(annotations, dict):
                                # Find the corresponding row in full_df by slice_id
                                current_video = annotated_df.iloc[review_idx]
                                slice_id = current_video['slice_id']
                                
                                # Find matching row in full_df
                                matching_rows = full_df[full_df['slice_id'] == slice_id]
                                if len(matching_rows) > 0:
                                    original_idx = matching_rows.index[0]
                                    
                                    # Update semantic group columns
                                    for group_name in semantic_columns:
                                        if group_name in annotations:
                                            ...
                                            full_df.loc[original_idx, group_name] = keywords_str
                        ...
                        save_columns = [col for col in columns_to_save if col in full_df.columns]
                        save_df_subset = full_df[save_columns]
                ...
                # Video info
                st.write(f"**Slice ID:** {current_review_video['slice_id']}")
                
                # Display span information
                if 'span_start' in current_review_video and 'span_end' in current_review_video:
                    span_start = current_review_video['span_start']
                    span_end = current_review_video['span_end']
                    duration = span_end - span_start
                    st.write(f"**Span:** {span_start}s - {span_end}s ({duration}s)")
                
                # Get folder name for context
                video_path = current_review_video['video_path']
                folder_name = Path(video_path).parent.name if pd.notna(video_path) else "Unknown"
                st.write(f"**Source:** {folder_name}")
                
                # Display category if available
                if 'category' in current_review_video and pd.notna(current_review_video['category']):
                    st.write(f"**Category:** {current_review_video['category']}")
                
                # Load existing annotations for this video into review session state
                if not st.session_state.review_annotations_loaded or st.session_state.review_index not in st.session_state.review_annotations:
                    current_annotations = {}
                    for group_name in semantic_columns:
                        if group_name in current_review_video and pd.notna(current_review_video[group_name]):
                            keywords_str = str(current_review_video[group_name])
                            if keywords_str.strip() and keywords_str != 'nan':
                                keywords = [k.strip() for k in keywords_str.split(',') if k.strip()]
                                current_annotations[group_name] = keywords
                            else:
                                current_annotations[group_name] = []
                        else:
                            current_annotations[group_name] = []
                    
                    st.session_state.review_annotations[st.session_state.review_index] = current_annotations
                    st.session_state.review_annotations_loaded = True
                
                # Semantic groups with their respective keywords (same as annotation interface)
                semantic_groups = {
                    'pv_object_type': [
                ...
                # Semantic group selections (same as annotation interface)
                st.subheader("🏷️ Edit Semantic Annotations")
                
                # Get current annotations for this video from review session state
                current_review_annotations = st.session_state.review_annotations.get(st.session_state.review_index, {})
                if not isinstance(current_review_annotations, dict):
                    current_review_annotations = {}
                ...
                for group_name, keywords in semantic_groups.items():
                    ...
                    current_selection = current_review_annotations.get(group_name, [])
                    ...
                    selected = st.multiselect(
                        ...
                        key=f"review_{group_name}_{st.session_state.review_index}",
                        help=f"Select {group_display_name.lower()} keywords that apply to this video"
                    )
                    
                    selected_annotations[group_name] = selected
                
                # Save current review annotations
                st.session_state.review_annotations[st.session_state.review_index] = selected_annotations
                ...
                with col_prev:
                    if st.button("⬅️ Previous", disabled=st.session_state.review_index == 0, key="review_prev"):
                        st.session_state.review_index -= 1
                        st.rerun()
                
                with col_next:
                    if st.button("➡️ Next", disabled=st.session_state.review_index >= len(annotated_df) - 1, key="review_next"):
                        st.session_state.review_index += 1
                        st.rerun()
                
                # Jump to specific video for review
                st.markdown("---")
                jump_to = st.number_input(
                    "Jump to video #:",
                    min_value=1,
                    max_value=len(annotated_df),
                    value=st.session_state.review_index + 1,
                    key="review_jump_input"
                )
                
                if st.button("🔄 Jump", key="review_jump"):
